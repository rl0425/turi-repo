...
import type { Pet, PetSpecies, PetSize, AdoptionStatus, PetAgeGroup, HealthStatus } from '@/types/pet';
...
function estimateSizeFromBreed(kindCd: string): PetSize {
  ...
  // 초소형견
  if (breed.includes('치와와') || breed.includes('요크셔') || breed.includes('말티즈') || 
      breed.includes('포메라니안') || breed.includes('파피용')) {
    return 'extra-small';
  }
  
  // 소형견
  if (breed.includes('시츄') || breed.includes('비숑') || breed.includes('코커') ||
      breed.includes('닥스훈트') || breed.includes('푸들') || breed.includes('웰시코기')) {
    return 'small';
  }
  
  // 대형견
  if (breed.includes('골든') || breed.includes('래브라도') || breed.includes('셰퍼드') ||
      breed.includes('허스키') || breed.includes('도베르만') || breed.includes('로트와일러')) {
    return 'large';
  }
  
  // 초대형견
  if (breed.includes('세인트') || breed.includes('그레이트') || breed.includes('마스티프') ||
      breed.includes('뉴펀들랜드') || breed.includes('아이리시')) {
    return 'extra-large';
  }
  
  // 고양이는 기본적으로 소형
  if (breed.includes('고양이') || breed.includes('코리안숏헤어') || breed.includes('페르시안') ||
      breed.includes('러시안블루') || breed.includes('브리티시숏헤어')) {
    return 'small';
  }
...
/**
 * 나이 정보를 기반으로 연령대 계산 (실제 API 형태: "2020(년생)")
 */
function calculateAgeGroup(age: string): { years: number; months: number; ageGroup: PetAgeGroup } {
  const currentYear = new Date().getFullYear();
  let years = 0;
  let months = 0;
  
  // "2020(년생)" 형태 파싱
  const birthYearMatch = age.match(/(\d{4})\(년생\)/);
  if (birthYearMatch) {
  ...
  // "3살", "1년" 등의 형태
  else if (age.includes('살') || age.includes('년')) {
...
function parseColors(colorCd: string): string[] {
  ...
  return colors.map(color => color.trim()).filter(color => color.length > 0);
}
...
export function convertAbandonmentAnimalToPet(
  ...
  const breedName = animal.kindFullNm?.split(']')[1]?.trim() || animal.kindNm || '반려동물';
